...
/**
  ...
  bool add3D(const Point &p); 
  bool search3D(const Point &p);
  // Prints out points with xlo <= x <= xhi, ylo <= y <= yhi, zlo <= z <= zhi 
  void rangeSearch3D(const int xlo, const int xhi,
    const int ylo, const int yhi, const int zlo, const int zhi);
...
public:
  ...
  ~RangeTree3D();
  // Prints out points with xlo <= x <= xhi, ylo <= y <= yhi, zlo <= z <= zhi 
  void rangeSearch(const int xlo, const int xhi,
    const int ylo, const int yhi, const int zlo, const int zhi);
};
